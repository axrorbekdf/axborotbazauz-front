...
<template>
    <section class="container max-w-screen-xl mx-auto py-12 px-4">
        <h2 v-if="loadingStore.isSearch" class="text-2xl font-bold text-purple-700 mb-6" style="color: #0A133C;">{{ t('qidirilmoqda') }}</h2>
        <section v-else>
          ...
          <div class="flex flex-col sm:flex-row sm:justify-between items-start sm:items-center gap-2 mb-6">
            <div>
              ...
              <h2 v-else class="text-2xl font-bold text-purple-700 mb-6" style="color: #0A133C;">{{ t('qidirishNatijalari') }}</h2>
            </div>
            <div>
          ...
          <div class="space-y-4">
              <!-- Single Item -->
              <NuxtLink :to="localeRoute('/document/material/'+item.slug)" v-for="item in modelStore.getMaterials as Array<any>" :key="item" class="flex flex-wrap items-center gap-4 p-4 border border-purple-400 rounded-lg">
